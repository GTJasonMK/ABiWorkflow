...
⚠️ 重要时长约束（必须遵守）：
- 当前视频 Provider 单次最长生成 {max_scene_seconds} 秒视频
- 若给定 allowed_scene_seconds（离散秒数列表），estimated_duration 必须从该列表选择：{allowed_scene_seconds}
- 若未给定 allowed_scene_seconds，则 estimated_duration 只需 ≤ {max_scene_seconds}
...
- 拆分时保持叙事连贯性，通过分镜标题体现顺序关系（如"告白-起身"、"告白-拥抱"）
...
NARRATIVE_ANALYSIS_USER = """请分析以下剧本，提取角色、分镜片段和全局风格信息。
再次提醒：每个场景的 estimated_duration 不得超过 {max_scene_seconds} 秒，超长叙事必须拆分。
如果提供了 allowed_scene_seconds（离散秒数列表），estimated_duration 必须从该列表选择：{allowed_scene_seconds}
